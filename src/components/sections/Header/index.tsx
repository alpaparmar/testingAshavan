...
function MobileMenu(props) {
    ...
    return (
        <div className="ml-auto lg:hidden">
            <button type="button" aria-label="Open Menu" title="Open Menu" className="p-2 -mr-1 focus:outline-none" onClick={() => setIsMenuOpen(true)}>
            ...
            <div className={classNames(secondaryColors, 'fixed', 'inset-0', 'px-4', 'sm:px-8', 'py-5', 'overflow-y-auto', 'z-20', isMenuOpen ? 'block' : 'hidden')}>
                <div className="flex flex-col min-h-full">
                    <div className="flex items-center justify-between mb-10">
                        ...
                        <button type="button" aria-label="Close Menu" title="Close Menu" className="p-2 -mr-1 focus:outline-none" onClick={() => setIsMenuOpen(false)}>
...
function listOfLinks(links, inMobileMenu = false, isSubLinks = false) {
    return links.map((link, index) => (
        <li key={index} className={classNames((link.subLinks && !inMobileMenu) ? 'dropdown' : null)}>
            <Action {...link} className={classNames(inMobileMenu && link.type === 'Button' ? 'w-full' : '', isSubLinks ? 'subLink' : '',
                (isSubLinks && index === 0 && inMobileMenu) ? 'mt-4' : '', (link.subLinks && !inMobileMenu) ? 'dropbtn' : '')}
                data-sb-field-path={`.${index}`} data-sb-inMobileMenu={inMobileMenu}/>
            {link.subLinks && (
                <ul className={classNames("flex-grow", "space-y-4", inMobileMenu ? "pl-4" : 'dropdown-content')}>
                    {listOfLinks(link.subLinks, inMobileMenu, true)}
                </ul>
            )}
        </li>
    ));
